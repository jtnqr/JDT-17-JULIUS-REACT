...
const Movies = () => {
	...
	// Debounce input to prevent API spamming
	useEffect(() => {
	...
	useEffect(() => {
		if (!debouncedQuery.trim()) {
			document.title = "Movies | JDT-17 Page";
			return;
		}
		...
		const performSearch = async () => {
			try {
				...
				if (isMounted) {
					const errMsg = err instanceof Error ? err.message : "Failed to search movies";
	...
	const isCurrentlyLoading = isSearching || (isNowPlayingLoading && !searchQuery);
	const currentError = searchQuery ? searchError : nowPlayingError;
	const currentMovies = searchQuery ? searchResults : nowPlayingMovies;
						...
						<div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 border-b border-zinc-900 pb-6">
							<div>
								...
								<h3 className="text-3xl font-bold tracking-tight text-zinc-50 sm:text-4xl">
									{searchQuery ? `Search Results for "${searchQuery}"` : "Now Playing in Theaters"}
								</h3>
							</div>
							...
							<div className="flex items-center gap-4 w-full md:w-auto shrink-0">
								...
								>
									{isCurrentlyLoading
										? "Loading..."
										: `${currentMovies.length} ${searchQuery ? "Matches" : "Movies"}`}
								</Badge>
							</div>
						</div>
						...
						{isCurrentlyLoading ? (
							...
							<Card className="border-dashed border-zinc-800 bg-zinc-950 p-16 text-center max-w-md mx-auto rounded-2xl flex flex-col items-center justify-center">
								...
								<CardDescription className="text-zinc-500 text-sm">
									We couldn't find any results matching "{searchQuery}". Try adjusting your
									keywords.
								</CardDescription>
							</Card>
						) : (
